...
export default function createStore(state) {
	...
	function setState(update, overwrite) {
		...
		for (let i=0; i<currentListeners.length; i++) {
			currentListeners[i]._callback(state);
			currentListeners[i]._effect(state);
		}
		...
		for (let j=0; j<stateChanged.length; j++) {
		  ...
		  if (onListeners.has(_key)) {
			...
			for (let k=0; k<listeners.length; k++) {
			  listeners[k]._callback(state[_key])
			  listeners[k]._effect(state[_key])
			}
		  }
		}
	}
	...
	function on(stateSlice, callback, effect = noop) {
		...
		const _call = { _callback: callback, _effect: effect }
	...
	return /** @lends store */ {
		...
		subscribe(listener, effect = noop) {
			const _listener = { _callback: listener, _effect: effect };
